from bioshift.core.peak import Peak
...
class Assignment:
    ...
    def __repr__(self) -> str:
        res = self.atom.get_parent()
        res_name = res.get_resname()
        res_index = res.get_id()[1]
        atom_name = self.atom.get_name()
        return f"<{res_name}{res_index}.{atom_name} : {self.peak.__repr__()}, confidence={self.confidence}>"
