...
@pytest.mark.parametrize('mismatched_transform_data', random_transform_data_mismatched())
...
def test_apply_inverse(transform_data, point):
    ...
    if point.shape == transform.inverse.shape:
        assert np.allclose(transform.inverse.apply(point),
                           (point - offset) / scaling)
    else:
        with pytest.raises(ValueError):
